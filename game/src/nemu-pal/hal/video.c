...
void SDL_BlitSurface(SDL_Surface *src, SDL_Rect *srcrect, 
	 ...
	 * `dst' surface.
	 */
	uint8_t *src_ptr = src->pixels + sy * src->w + sx;
	uint8_t	*dst_ptr = dst->pixels + dy * dst->w + dx;
	//Log("src_ptr = %x\n", (uint32_t)src_ptr);
	//Log("dst_ptr = %x\n", (uint32_t)dst_ptr);
	for(int i = 0; i < h; i++){
...
void SDL_FillRect(SDL_Surface *dst, SDL_Rect *dstrect, uint32_t color) {
	...
	if(dstrect == NULL){
		...
		uint8_t *pix_ptr = dst->pixels + dstrect->y * dst->w + dstrect->x;
		for(int i = 0; i < dstrect->h; i++){
			memset(pix_ptr, color, dstrect->w);
			pix_ptr += dst->w;		
		}
	}
	//assert(0);
