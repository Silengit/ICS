...
inline uint32_t internal_normalize(uint32_t sign, int32_t exp, uint64_t sig_grs) {
	...
	if((sig_grs >> (23 + 3)) > 1 || exp < 0) {
		//printf("f.sign=%x,exp_res=%x, sig_res=%llx\n",sign,exp, sig_grs);
...
uint32_t internal_float_add(uint32_t b, uint32_t a) {
	...
	if(fa.exponent > fb.exponent) {
		fa.val = b;
		fb.val = a;
	}//fb.exponent>fa.exponent  for ever
	...
	exp_res = fb.exponent;
	//printf("f.sign=%x,exp_res=%x, sig_res=%x\n",f.sign,exp_res, sig_res);
...
void fpu_sub(uint32_t val) {
	...
	fpu.regStack[fpu.status.top].val = *d;
	*/
	//printf("%x\n",fpu.regStack[fpu.status.top].val);//delete
	//printf("%x\n",val);
